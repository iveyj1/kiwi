...
User validation found the cursor step functional and the current slice acceptable. Final-product requirement: tuning selection must advance on configured round frequency steps, not FFT/source-bin increments. Future cursor state will therefore retain an exact selected frequency on the tuning grid and project it onto the nearest raster column; bin width remains a resolution diagnostic. This change must precede cursor-driven W/F or SND commands.
