...
Added coordinated but separately owned SND audio to `kiwi-wf-terminal`. `a` starts/stops a `play_live_snd()` task, Enter queues an exact `SET mod` tune, `--audio`/`[waterfall].audio` starts audio with the viewer, and `--null-audio` provides device-free diagnostics. Audio config inherits receiver policy, limits, fades, mode/passband, command precision, and CW offset. Audio OFF/STARTING/ON/STOPPING/ENDED/ERROR appears in status; runner errors are isolated from W/F and viewer shutdown stops both tasks. Fake-runner tests cover toggle, exact AM tune, error isolation, and CW radio offset; dry-run exposes the full SND plan. Full harness: 275 passed in 3.18 seconds. No live receiver or audio device was used.

First proxy test exposed W/F closure code 1005 immediately after delayed audio toggle. Upstream browser `open_websocket()` uses one `kiwi.conn_tstamp` for SND and W/F, whereas both project configs previously retained `timestamp=None` and each URI generated its own current timestamp. The viewer now assigns one id before constructing either config and reuses it for delayed audio. Explicit timestamps are preserved. Shared/delayed URI regressions pass; full harness: 277 passed in 3.16 seconds. No automatic external retest was made.
