...
The controller opens SND before W/F, publishes bounded latest snapshots, routes typed tune/view actions to the active stream queues, and stops/joins both streams on console exit. The lower panel shows live SND RSSI in dB, a conventional HF S-unit estimate (`S9 = -73 dBm`, 6 dB per S-unit below S9), and a bounded `-130..-20 dB` bar. `--dry-run` reports the intended command without connecting.
