...
store all a    # receiver/tuning/filter/AGC preset in letter register a
recall a
```

In keymap mode, press `s1`, `Sa`, or `pa` for the same store/store-all/recall operations. After `p`, `s`, or `S`, defined preset registers are shown with frequency and mode. Add stored receiver registers with `add-receiver 2 10.0.0.42:8073 Backup receiver` or alias `ad a http://10.0.0.42:8073/ Backup receiver`; URL-like addresses are normalized to `host:port`. Newly added receivers are appended to `[receivers].allowed` in the active config file if not already present. Receiver prefix `r <receiver>` switches to a stored receiver if receiver presets exist, otherwise to `[receivers].allowed` using register order `0..9,a..z`, while preserving frequency/mode/filter and other radio parameters; after `r`, hints show stored receiver registers only when present, or config allowlist registers when no stored receiver presets exist. If background playback is active, the TUI stops it and restarts playback on the selected receiver. If playback had already failed, for example because the current receiver was busy, `r <receiver>` switches receiver and starts playback on the selected receiver. If the new receiver fails immediately, such as reporting busy, the TUI restores the previous receiver, restarts playback there when possible, and shows the failure message. The TUI persists durable radio and receiver-register presets to `[presets].file`; `[startup].state_file` stores only ephemeral `last_state`. On restart, `[startup].mode` controls whether to use `last`, `default`, or a configured `preset`.
...
The combined live viewer fetches the receiver's `/VER` metadata, opens `/ws/kiwi/<server timestamp>/SND`, waits for the receiver's successful `badp=0` authentication response, and only then opens paired W/F with the same timestamp, matching current Kiwi browser startup. Without `--audio` / `[waterfall].audio = true`, the SND stream remains connected but the lazy local sink is `MUTED`; `a` toggles local output without tearing down the primary SND session. `--null-audio` keeps output device-free even when toggled ON. Enter sends `SET mod=...` for the exact cursor frequency, mode, and passband. CW preserves the project convention that cursor/user frequency is the signal/passband-center reference and applies configured `cw_offset_hz` to radio frequency. The graphical passband uses the actual RF edges, `user frequency + cw_offset_hz + low/high cut`; CW status separately shows `nominal` and `RX`, and the amber RX marker is placed at `nominal + cw_offset_hz`; its center therefore differs from the user marker if the offset magnitude does not equal the midpoint of the configured CW cuts. A primary SND startup failure prevents opening orphan W/F; later audio-device errors are shown in status.
...
The controller completes authenticated SND startup before W/F, publishes bounded latest snapshots, routes typed tune/view actions to the active stream queues, and stops/joins both streams on console exit. WebSocket close handshakes allow one second before forced teardown to reduce orphaned server allocations during receiver switching. If SND stalls while a stop fade is pending, the next receive timeout abandons the fade; paired teardown also cancels a primary runner that has not stopped within one second, so a frozen old stream cannot permanently block receiver replacement. Press `a` to lazily open or close local audio output without tearing down the primary SND connection; the selected state survives receiver restart. Audio-device startup failure leaves the console muted and reports an error. Receiver restart replaces the bounded publisher; the console detects that identity change, clears old raster history/placement, and binds the new receiver's publisher before drawing its first frame. The lower panel shows fixed-width live SND RSSI in dBm, a conventional HF S-unit estimate (`S9 = -73 dBm`, 6 dB per S-unit below S9), and a bounded `-130..-20 dBm` bar. Fixed numeric/S-unit fields prevent the surrounding status line from jumping as signal strength changes. `--dry-run` reports the intended command without connecting.

Fixture/live tuning defaults to simple immediate mode with 1.0/0.1 kHz main/fine steps (`--main-step-khz` / `--small-step-khz`). Live startup selects the nearest configured mode step pair, and `t`/`T` cycle larger/smaller configured pairs while synchronizing the waterfall model. `h`/`l` or arrows move and immediately tune by the main step; `H`/`L` do the same with the fine step. Crossing a visible W/F edge automatically recenters on the new frequency, and live startup centers W/F on the restored/startup frequency instead of the static waterfall configuration center. Incremental/direct tuning, audio toggle, explicit recenter, and zoom remain available while waiting for the first W/F frame. Because no viewport mapping exists yet, automatic edge recenter is deferred rather than treating the missing frame as fatal. Other controls are Enter to resend the current tune, `c` to center explicitly, `+`/`-` to zoom, `f` for direct kHz entry, `p` followed by a register to recall a preset locally, and `q` to exit. Generated radio commands remain local and are not sent. Direct frequency entry tunes SND and automatically recenters W/F only when the entered frequency is outside the currently visible range; an in-range entry leaves the viewport unchanged. Enter resends the tune without recentering, and `c` always centers explicitly. Up arrow/Page Up tunes one visible span higher and recenters; Down arrow/Page Down tunes one visible span lower and recenters. The upper Kitty image now composes W/F history, a two-pixel green passband/nominal-frequency bracket, an amber CW receiver-frequency marker, magenta selection pointer, major frequency tick stems, ten equal short-tick intervals between adjacent labels, anti-aliased FreeType labels, and cyan preset stems/labels on one 1024-bin graphical coordinate system. Labels omit `kHz`, non-major exact edges are suppressed, and pixel-coordinate collision rejection keeps labels readable. Passbands intersecting a viewport edge are clipped at that edge instead of disappearing. `--scale-font auto` uses Pillow with DejaVu Sans Mono and falls back to the deterministic bitmap font; `--scale-font bitmap` forces the fallback and `--scale-font-name` selects another installed TrueType font. Redundant terminal-cell rulers have been removed. This follows the general KiwiSDR visual language without duplicating the browser UI.

Mouse input is intentionally ignored in the integrated console. In particular, wheel events are consumed instead of being allowed to fall back to terminal up/down arrow sequences and accidentally change volume.

In live mode, `:` opens the existing TUI command editor with history and contextual hints. `m` opens a receive-mode prefix map: `a` AM, `u` USB, `l` LSB, and `c` CW. Mode selection applies the existing configured per-mode passband and step pairs and sends the normal active SND modulation command. Configured non-waterfall key actions, preset/store prefixes, receiver registers, `k`/`j` volume keys, command parsing, and errors delegate to the existing TUI/controller code; current volume is shown beside RSSI. waterfall selection/tune/center and display-level keys remain console-specific.

Display levels start from `--render-min-db` / `--render-max-db`. `u` toggles smoothed percentile auto-scale; `[`/`]` lower/raise the manual minimum by 5 dB and `{`/`}` lower/raise the manual maximum. Any manual adjustment disables auto mode. Current mode and limits are shown in status.
...
- `f`: focus direct kHz entry; **Set frequency** updates tuned/selected/center state and returns focus to the waterfall.
...
Use `--tuned-khz`, `--mode`, `--low-cut-hz`, and `--high-cut-hz` to set initial overlay/radio state. Fixture recenter/zoom now remap captured pixels to the local viewport; zooming out beyond captured coverage fills unavailable frequencies with black. This is a visual control harness, not synthesized receiver data.
