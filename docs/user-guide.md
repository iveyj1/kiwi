...
In live mode, `:` opens the existing TUI command editor with history and contextual hints. Configured non-waterfall key actions, preset/store prefixes, receiver registers, volume keys, command parsing, and errors delegate to the existing TUI/controller code; waterfall selection/tune/center and display-level keys remain console-specific.
