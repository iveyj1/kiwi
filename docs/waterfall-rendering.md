...
- `WaterfallCursor` keeps exact selected frequency independently from source bins, snaps stepped movement to zero-anchored configured frequency steps, clamps to the mapped span, and preserves frequency across resolution/context changes. The magenta marker projects selection onto the nearest raster column. Interactive `f` entry accepts an exact kHz value, tunes SND, recenters W/F at the current zoom, and moves the cursor without step-grid rounding once the new mapped span contains it.
...
- `WaterfallSnapshotPublisher` provides a separate renderer-neutral frontend boundary: immutable bounded numeric rows retain sequence, monotonic arrival time, and each row's original frequency coordinates. Consumers wait by generation and receive only the latest state when behind, preventing an image-update queue while preserving data needed for future zoom remapping.
